...
import {measureGetResponseTime} from "../../utils/api-utils";
...
test("response time test utils kullanimi", async ({ request }) => {

  const responseTime= await measureGetResponseTime(request, "https://petstore.swagger.io/v2/pet/5");
  expect(responseTime).toBeLessThan(2000);
